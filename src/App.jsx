...
function App() {
  ...
  const getGreeting = () => {
    ...
    if (hour < 12) return { text: 'Good Morning', icon: <Sun className="w-6 h-6 text-[#f5f5dc]" /> };
    if (hour < 18) return { text: 'Good Afternoon', icon: <Sun className="w-6 h-6 text-[#f5f5dc]" /> };
    return { text: 'Good Evening', icon: <Moon className="w-6 h-6 text-[#f5f5dc]" /> };
  };
              ...
              <div className="p-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex items-start space-x-3">
                    <Terminal className="w-5 h-5 text-[#f5f5dc]" />
                    <div>
                      <p className="text-[#f5f5dc] font-mono">SUBJECT:</p>
                      <h2 className="text-[#f5f5dc] text-xl font-bold">{exam.exam}</h2>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <Calendar className="w-5 h-5 text-[#f5f5dc]" />
                    <div>
                      <p className="text-[#f5f5dc] font-mono">DATE:</p>
                      <p className="text-[#f5f5dc]">{exam.date}</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <Clock className="w-5 h-5 text-[#f5f5dc]" />
                    <div>
                      <p className="text-[#f5f5dc] font-mono">TIME:</p>
                      <p className="text-[#f5f5dc]">{exam.time}</p>
                    </div>
                  </div>
                  <div className="flex items-start space-x-3">
                    <Quote className="w-5 h-5 text-[#f5f5dc]" />
                    <div>
                      <p className="text-[#f5f5dc] font-mono">NOTES:</p>
                      <p className="text-[#f5f5dc]">{exam.quote}</p>
                    </div>
                  </div>
                </div>
                <button
